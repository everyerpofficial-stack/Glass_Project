...
(function (root, factory) {
  ...
  function trunc(v, d) {
    var f = Math.pow(10, d || 0);
    return Math.trunc(v * f + 1e-9) / f;
  }
  function fix(v, d, mode) {
    return mode === "truncate" ? trunc(v, d) : round(v, d);
  }
  function money(v) {
    return round(v, 2);
  }
  ...
  var DEFAULTS = {
    ...
    extraAreaFormula: "+25mm", // 'none' | '+25.4mm' | '+25mm' | '+50mm' | 'custom'
    ...
    commissionValue: 0,
    commissionBase: "basic", // 'basic' | 'glass' | 'grand'
    roundOff: true,
  ...
  var PRESETS = {
    anand: {
      // Anand Lamps Pvt Ltd / Hindustan Float Glass
    ...
    krishna: {
      ...
      rateUnit: "sqft",
      chargeAllowanceMM: MM_PER_INCH,
      areaDecimals: 3,
      areaRoundMode: "truncate",
      adminCharge: 0,
      insurancePercent: 0,
      cgstPercent: 0,
      sgstPercent: 0,
  ...
  function settings(over) {
    ...
    for (k in over || {})
      if (over[k] !== undefined && over[k] !== null && over[k] !== "") s[k] = over[k];
    return s;
  }
  ...
  function parseMM(raw) {
    ...
    if (!/^\d+(\.\d+)?$/.test(t))
      return { ok: false, mm: 0, input: t, error: 'Only numbers and "." are allowed for MM input' };
    var v = parseFloat(t);
    if (v <= 0) return { ok: false, mm: 0, input: t, error: "Size must be greater than 0" };
    if (v > 10000)
      return { ok: false, mm: 0, input: t, error: "Size looks too large (over 10000 mm)" };
  ...
  function parseInch(raw, opts) {
    ...
    function frac(w, n, d, txt) {
      ...
      if ([2, 4, 8, 16, 32, 64].indexOf(d) === -1)
        return err("Use halves, quarters, eighths, 1/16, 1/32 or 1/64");
      return ok(w + n / d, txt);
    }
    ...
    function err(msg) {
      return { ok: false, inches: 0, input: t, note: null, error: msg };
    }
  }
  ...
  function inchToMM(inches, dec) {
    return round(inches * MM_PER_INCH, dec == null ? 2 : dec);
  }
  function mmToInch(mm) {
    return mm / MM_PER_INCH;
  }
  function sqmToSqft(a) {
    return a * SQFT_PER_SQM;
  }
  function sqftToSqm(a) {
    return a / SQFT_PER_SQM;
  }
  ...
  function calcLine(item, opts) {
    ...
    if (inputUnit === "mm") {
      ...
      L = parseInch(item.l1, inchOpts);
      W = parseInch(item.l2, inchOpts);
      out.l1 = L;
      out.l2 = W;
    ...
    if (s.cuttingAllowanceMM) {
      trace.push({
        label: "Cutting size",
        expr: "each side + " + s.cuttingAllowanceMM + " mm",
    ...
    var billingArea = effectiveChargeAllowance
      ? unit === "sqft"
        ? out.chargeAreaSqft
        : out.chargeAreaSqm
      : unit === "sqft"
  ...
  function unitLabel(u) {
    return u === "sqft" ? "Sq.Ft" : u === "piece" ? "pc" : "Sq.Mtr";
  }
  ...
  function calcInvoice(items, opts) {
    ...
    valid.forEach(function (l) {
      ...
      t.sqm += l.totalSqm;
      t.sqft += l.totalSqft;
      t.chargeSqm += l.chargeAreaSqm;
      t.chargeSqft += l.chargeAreaSqft;
      t.glassAmount += l.amount;
      t.holes += l.holes;
      t.cutouts += l.cutouts;
      t.bigCutouts += l.bigCutouts;
      t.countersinks += l.countersinks;
      t.csks += l.csks;
      t.bigHoles += l.bigHoles;
    });
    ...
    // wastage
    t.wastageArea = 0;
    if (s.wastageMode === "percent")
      t.wastageArea = round((t.sqm * (parseFloat(s.wastagePercent) || 0)) / 100, 3);
    ...
    // wastage 2
    t.wastage2Area = 0;
    if (s.wastage2Mode === "percent")
      t.wastage2Area = round((t.sqm * (parseFloat(s.wastage2Percent) || 0)) / 100, 3);
    else if (s.wastage2Mode === "manual")
      t.wastage2Area = round(parseFloat(s.wastage2Area) || 0, 3);
    ...
    t.basicAmount = money(
      t.glassAmount +
        ...
        t.jamboCharge +
        t.nonEconomicCharge +
        t.farmaCuttingCharge +
        t.shapeCuttingCharge +
        t.katraPolishCharge +
        t.designCharge +
        t.screenPrintCharge +
        t.bewalingCharge +
    ...
    t.subTotal = money(
      t.basicAmount + t.adminCharge + t.freight + t.unloadingHandling + t.greenTax - t.discount,
    );
    ...
    var base =
      s.commissionBase === "glass"
        ? t.glassAmount
        : s.commissionBase === "grand"
          ? preTcsGross
          : t.basicAmount;
    t.commission =
      s.commissionMode === "percent"
        ? money((base * (parseFloat(s.commissionValue) || 0)) / 100)
        : s.commissionMode === "fixed"
          ? money(parseFloat(s.commissionValue) || 0)
          : 0;
