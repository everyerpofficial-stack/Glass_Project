...
(function (root, factory) {
  ...
  function run() {
    ...
    check(
      'sooth mode 13.6 = 13.75"',
      G.parseInch("13.6", { inputMode: "sooth" }).inches,
      13.75,
      1e-9,
    );
    check("sooth mode rejects 13.9", G.parseInch("13.9", { inputMode: "sooth" }).ok, false);
    check(
      'frequency 16 mode 36 2 = 36.125"',
      G.parseInch("36 2", { freq: 16 }).inches,
      36.125,
      1e-9,
    );
    check(
      'frequency 16 mode 32 2 = 32.125"',
      G.parseInch("32 2", { freq: 16 }).inches,
      32.125,
      1e-9,
    );
    ...
    var pi1813 = G.calcLine(
      { l1: "95 7/8", l2: "40 7/8", qty: 2, rate: 1344 },
    ...
    var krRows = [
      // L mm, W mm, qty, Tot.Chg S.F., Glass Amount
      [474, 751, 2, 8.347, 575.94],
      [775, 276, 1, 2.596, 179.12],
      [777, 2202, 2, 38.475, 2654.78],
      [495, 1108, 2, 12.697, 876.09],
      [919, 2241, 2, 46.077, 3179.31],
      [869, 1330, 2, 26.097, 1800.69],
      [518, 608, 1, 3.704, 255.58],
      [298, 276, 1, 1.049, 72.38],
      [646, 1326, 1, 9.766, 673.85],
      [1135, 1318, 2, 33.559, 2315.57],
      [286, 1326, 1, 4.529, 312.5],
      [469, 583, 1, 3.237, 223.35],
      [249, 276, 1, 0.89, 61.41],
    ];
    var krTotal = 0;
    krRows.forEach(function (r) {
      // the Krishna sheet is driven from mm, so feed mm back as inches
      var L = G.calcLine(
        { l1: String(G.mmToInch(r[0])), l2: String(G.mmToInch(r[1])), qty: r[2], rate: 69 },
    ...
    var tot = G.calcInvoice(
      items1828,
      G.settings(
        Object.assign({}, G.PRESETS.anand, {
          wastageMode: "manual",
          wastageArea: 1.692,
          wastageRate: 657,
          adminCharge: 50,
    ...
    var sub = G.money(basic + 50);
    check("total after admin", sub, 11453.28, 0.01);
    var ins = G.money(sub * 0.02);
    check("insurance 2%", ins, 229.07, 0.01);
    var ass = G.money(sub + ins);
    check("assessable value", ass, 11682.35, 0.01);
    var cg = G.money(ass * 0.09);
    check("C-GST 9%", cg, 1051.41, 0.01);
    var gross = G.money(ass + cg * 2);
    check("gross total", gross, 13785.17, 0.01);
    ...
    var s13 = G.money(b13 + 50),
      i13 = G.money(s13 * 0.02),
      a13 = G.money(s13 + i13);
    ...
    check("PI-1813 weight 12mm", G.round(5.062 * 12 * 2.5, 3), 151.86, 0.01);
    ...
    check("12.7530 Sq.M in Sq.Ft", G.round(G.sqmToSqft(12.753), 3), 137.269, 0.005);
    check("5.0620 Sq.M in Sq.Ft", G.round(G.sqmToSqft(5.062), 3), 54.486, 0.005);
    ...
    check(
      "words 13785",
      G.amountInWords(13785),
      "Thirteen Thousand Seven Hundred Eighty Five Only.",
    );
    check("words 100000", G.amountInWords(100000), "One Lakh Only.");
    check(
      "words 12345678",
      G.amountInWords(12345678),
